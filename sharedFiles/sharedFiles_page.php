 ...
 <table class="table table-striped table-hover">
        <thead>
            <tr>
            	<th>File Number</th>
              <th>File owner</th>
                <th>File shared with</th>
                <th>File Hash</th>
                
            </tr>
<?php

// SELECT * FROM `sharedFiles` WHERE (`owner_username` = 'noel' OR `shared_username` = 'username' ) GROUP BY `file_hash`

$sql = "SELECT * FROM `sharedFiles` WHERE (`owner_username`  = (:username) OR `shared_username` = (:username)) GROUP BY `file_hash";
        ...
        if($stmt->rowCount() >= 1)
        {
            ...
            foreach ($records as $record) {
                ...
                echo "</td>";
                 ...
                 echo "<td>";
                echo $record['owner_username']; // It is a file name
                $file_name[] = $record['owner_username'];
                echo "</td>";


                echo "<td>";
                echo $record['shared_username']; // It is a file name
                echo "</td>";

                echo "<td>";
                echo $record['file_hash'];
                $file_hash[] = $record['file_hash']; // It is a file hash
                echo "</td>";
